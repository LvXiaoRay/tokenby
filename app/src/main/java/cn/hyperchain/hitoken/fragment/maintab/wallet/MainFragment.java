...
public class MainFragment extends BaseBarFragment implements com.rthtech.ble.Callback {
    ...
    @BindView(R.id.tv_reissue_wallet)
    TextView tvReissueWallet;
    ...
    private Controller rController = null;
    private Ble ble;
    private Ble ble2;
...
//                                cardSysResult.getCareerKey();
                ...
                tvReissueCard.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View view) {
                        ...
                        if(findCard == 1) {
                            ...
                            if(ble.checkSessionStatus()) {
                                showDialog();
                                cardMngIfs.submitReplaceInfoToSmart("13012345678", "00QQ11WW22EE33RR44", new CardMngIfs.AnsyCallback3<String>() {
                                    @Override
                                    public void AnsyLoader(String loder, String Url4) {
                                        ble.recoveryKey("","");//补卡操作，user key 和 平台 seed + PIN码
                                    }
                                });
                            } else {
                                showToast("No device connected!");
    ...
    @OnClick({
            ...
            R.id.iv_add_wallet,R.id.root,R.id.fl_content,R.id.tv_reissue_wallet
...
//                getActivity().getWindow().setAttributes(lp2);
            ...
            case R.id.tv_reissue_wallet:
                ...
                String bluetooth2 = "7C:01:0A:FC:7A:E7";

                ble = new Ble(mController);
                ble.conect(bluetooth2);
                showToast("设备连接中...");
                showDialog();
                WindowManager.LayoutParams lp4=getActivity().getWindow().getAttributes();
                //设置透明度
                lp4.alpha=0.3f;
                //弹窗位置
                cardReissueWindow.showAtLocation(root, Gravity.CENTER, 0, 0);
                //背后窗口参数
                getActivity().getWindow().addFlags(WindowManager.LayoutParams.FLAG_DIM_BEHIND);
                getActivity().getWindow().setAttributes(lp4);
